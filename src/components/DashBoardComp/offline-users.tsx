...
export function OfflineUsersList({ data }: OfflineUsersListProps) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-3xl font-bold tracking-tight">Offline Users</h2>
                    <p className="text-muted-foreground mt-1">
                        {users.length} {users.length === 1 ? "user" : "users"} currently offline
                    </p>
                </div>
            ...
            {users.length === 0 ? (
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {users.map((user) => (
                        <Card key={user._id} className="overflow-hidden bg-gray-700">
